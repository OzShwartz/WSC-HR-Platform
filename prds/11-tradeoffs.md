...
**Storage & scoring compute.** Today, `list_candidate_pool()` re-scores every candidate against every job on every request, straight from CSV files read fresh each time. That's fine at 75×5; it doesn't survive thousands of candidates × hundreds of open jobs. Production needs the `candidate_job_scores` table from the ERD ([presentation/png/erd-diagram.png](../presentation/png/erd-diagram.png)) populated by an async event, not a synchronous request: score a candidate against all open jobs once when they're captured, score all candidates against a job once when it opens, and serve reads from that cache. Recomputation only happens when a job's requirements or the scoring weights change.
